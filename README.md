...
### 架构分层说明
1.  **前端层（Frontend）**
    采用 React + TypeScript + Tailwind CSS 构建单页应用，通过 Axios 与后端进行 HTTP/REST 通信，为用户提供直观的学习交互界面，包括对话式画像构建、资源生成展示、学习路径可视化等功能。
...
3.  **数据层（Database）**
    - **Redis**：用于存储会话状态、对话历史缓存，提升多轮对话响应速度
    - **PostgreSQL**：持久化存储用户画像、学习记录、资源数据等核心业务数据
    数据库同样通过 Docker 容器化部署，确保开发与生产环境一致性。

4.  **基础设施层（Infrastructure）**
    基于云服务器 + Docker Engine 实现容器化部署，通过 Nginx 反向代理实现域名访问、负载均衡与静态资源分发，使用 Docker Compose 完成多容器编排与一键启动，保障系统可扩展性与稳定性。